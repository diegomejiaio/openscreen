...
describe("checkLatestRelease", () => {
	it("reports a newer official stable release", async () => {
		const fetchLatest = vi.fn().mockResolvedValue(
			releaseResponse({
				tag_name: "v1.10.0",
				html_url: "https://github.com/diegomejiaio/openscreen/releases/tag/v1.10.0",
				draft: false,
				prerelease: false,
			}),
		);
		...
		await expect(checkLatestRelease({ currentVersion: "1.9.0", fetchLatest })).resolves.toEqual({
			...
			releaseUrl: "https://github.com/diegomejiaio/openscreen/releases/tag/v1.10.0",
		});
		expect(fetchLatest).toHaveBeenCalledWith(
			"https://api.github.com/repos/diegomejiaio/openscreen/releases/latest",
	...
	it("reports current when the installed version is equal or newer", async () => {
		const fetchLatest = vi.fn().mockResolvedValue(
			releaseResponse({
				tag_name: "v1.9.0",
				html_url: "https://github.com/diegomejiaio/openscreen/releases/tag/v1.9.0",
				draft: false,
				prerelease: false,
			}),
		);
	...
	it.each([
		...
		const fetchLatest = vi.fn().mockResolvedValue(
			releaseResponse({
				tag_name: "v2.0.0",
				html_url: "https://github.com/diegomejiaio/openscreen/releases/tag/v2.0.0",
				draft,
				prerelease,
			}),
		);
	...
	it.each([
		"https://github.com/diegomejiaio/openscreen/releases/download/v9.9.9/app.zip",
		"https://github.com/diegomejiaio/openscreen/releases/tag/v9.9.8",
		"https://github.com/diegomejiaio/openscreen/releases/tag/v9.9.9?download=1",
		"https://github.com/diegomejiaio/openscreen/releases/tag/v9.9.9#notes",
